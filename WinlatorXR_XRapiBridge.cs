// WinlatorXR_XRapiBridge.cs
// Place compiled DLL into: Gorilla Tag\BepInEx\plugins\WinlatorXR_XRapiBridge.dll
...
[BepInPlugin("com.yourname.winlatorxr.xrapi", "WinlatorXR XRapi Bridge", "0.1.0")]
public class WinlatorXR_XRapiBridge : BaseUnityPlugin
{
    ...
    private void SendPoseAndInputs()
    {
        ...
        if (leftObj != null)
        {
            lPos = leftObj.transform.position;
            lRot = leftObj.transform.rotation;
        }
        if (rightObj != null)
        {
            rPos = rightObj.transform.position;
            rRot = rightObj.transform.rotation;
        }
        if (camT != null)
        {
            hPos = camT.position;
        ...
        floats[11] = rRot.z;
        floats[12] = rRot.w;
        floats[13] = rightStickX;
        floats[14] = rightStickY;
        floats[15] = rPos.x;
        floats[16] = rPos.y;
        floats[17] = rPos.z;
        ...
        floats[26] = 90f;
        floats[27] = 90f;
        floats[28] = (float)(_frameCounter & 0x7fffffff);

        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 29; i++)
        {
            sb.Append(floats[i].ToString("R"));
            if (i < 28) sb.Append(' ');
        }

        sb.Append(' ');
        sb.Append(BoolToTF(L_GRIP));
        sb.Append(BoolToTF(L_MENU));
        sb.Append(BoolToTF(L_THUMBSTICK_PRESS));
        sb.Append(BoolToTF(L_THUMBSTICK_LEFT));
        sb.Append(BoolToTF(L_THUMBSTICK_RIGHT));
        sb.Append(BoolToTF(L_THUMBSTICK_UP));
        sb.Append(BoolToTF(L_THUMBSTICK_DOWN));
        sb.Append(BoolToTF(L_TRIGGER));
        sb.Append(BoolToTF(L_X));
        sb.Append(BoolToTF(L_Y));

        sb.Append(BoolToTF(R_A));
        sb.Append(BoolToTF(R_B));
        sb.Append(BoolToTF(R_GRIP));
        sb.Append(BoolToTF(R_THUMBSTICK_PRESS));
        sb.Append(BoolToTF(R_THUMBSTICK_LEFT));
        sb.Append(BoolToTF(R_THUMBSTICK_RIGHT));
        sb.Append(BoolToTF(R_THUMBSTICK_UP));
        sb.Append(BoolToTF(R_THUMBSTICK_DOWN));
        sb.Append(BoolToTF(R_TRIGGER));
        ...
        try
        {
            ...
            if ((_frameCounter & 255) == 0)
                Logger.LogDebug("[XRapiBridge] Sent UDP length " + bytes.Length);
        }
        catch (Exception e)
        {
    ...
    private string BoolToTF(bool v) { return v ? "T" : "F"; }
